...
from .IpTablesLex import tokens
from .IpTablesLex import lexer
...
from .SpringBase.TCP_Flag import TCP_Flag
...
def init(name, raise_on_error=False):
    ...
    # create default acl
    p_info['firewall'].acl.append(ACL('INPUT', 'filter'))
    p_info['firewall'].acl.append(ACL('FORWARD', 'filter'))
    p_info['firewall'].acl.append(ACL('OUTPUT', 'filter'))
    # init parser state
...
def p_chain_line1(p):
    ...
    table = p_info['current_table']
    # if p_info['current_table'] == 'filter' and not find_chain_by_name(p[2]):
    if not find_chain_by_name(p[2]):
        p_info['firewall'].acl.append(ACL(p[2], table))
...
def p_chain_line3(p):
    ...
    print('in chain line 3', p[2])
    table = p_info['current_table']
    print('table =', table)
    # if p_info['current_table'] == 'filter' and not find_chain_by_name(p[2]):
    if not find_chain_by_name(p[2]):
        print('looking for acl ', p[2])
        p_info['firewall'].acl.append(ACL(p[2], table))
        print(p_info['firewall'].acl[-1].name)
...
def p_append_cmd(p):
    ...
    print('in append command with chain', p[2])
...
def p_tcp_flags(p):
    '''tcp_flags : TCP_FLAGS tcp_flag_list tcp_flag_list'''
    print('in p_tcp_flags', list(p))
    for v1, v2 in p[2]:
        print(v1, v2)
        if not v2:
            p_info['current_rule'].tcp_flag_list.append(TCP_Flag(v1))
        else:
            p_info['current_rule'].tcp_flag_list.append(TCP_Flag(v1), TCP_Flag(v2))
    for v1, v2 in p[3]:
        print(v1, v2)
        if not v2:
            p_info['current_rule'].tcp_flag_sets.append(TCP_Flag(v1))
        else:
            p_info['current_rule'].tcp_flag_sets.append(TCP_Flag(v1), TCP_Flag(v2))


# def p_tcp_flags_2(p):
#     '''tcp_flags : BANG TCP_FLAGS tcp_flag_list'''
#     for v1, v2 in p[3]:
#         if not v2:
#             p_info['current_rule'].flag_dest.append(TCP_Flag(v1))
#         else:
#             p_info['current_rule'].flag_dest.append(TCP_Flag(v1), TCP_Flag(v2))
#
#


def p_tcp_flag_list_1(p):
    '''tcp_flag_list : item COMMA tcp_flag_list'''
    print('in p_tcp_flag_list_1', list(p))
    p[0] = [(get_value(p[1]), None)] + p[3]


def p_tcp_flag_list_2(p):
    '''tcp_flag_list : item'''
    print('in p_tcp_flag_list_2', list(p))
    p[0] = [(get_value(p[1]), None)]
