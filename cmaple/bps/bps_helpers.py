...
# Create a logger for cmaple.bps.bps_helpers...
logger = logging.getLogger(re.sub('\.[^.]+$','',__name__))


@logged(logger)
@traced(logger)
def get_all_reference_dicts(BPS_instance):
    ...
    all_API_paths_list = []
    # Get the base paths
    resource_tuple_list = tree_helpers.get_jsonpath_full_paths_and_values('$.features.*.[*]',BPS_instance._json_dict)
